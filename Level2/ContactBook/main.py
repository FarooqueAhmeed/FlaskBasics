...
def register():
    if request.method=='POST':
        ...
        cur.execute("select * from contact_users where username=%s", (username,))
        data = cur.fetchall()
        if data:
            flash('username/password Already exist')
            return redirect('/')
        else:
            cur.execute("INSERT INTO contact_users(username,email,password) VALUES(%s,%s ,%s)", (username, email, password))
...
def login():
    if request.method == 'POST':
        ...
        password=request.form['password']
        cur=mysql.connection.cursor()
        cur.execute("SELECT * FROM contact_users WHERE username=%s AND password=%s",(username,password,))
        data=cur.fetchone()
        mysql.connection.commit()
        if data:
