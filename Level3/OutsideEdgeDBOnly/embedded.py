...
def query_mysql(query):
	cnx = mysql.connector.connect(user='root', password='yaKhudaKhair',
...
def get_indian_players(connection, team_name):
	...
	result = None
	try:
		cursor.execute("SELECT player_name from Player WHERE team_name=\"" + team_name + "\" and nationality='India'")
		result = cursor.fetchall()
		return result
	except Error as e:
		print(f"The error '{e}' occurred")
...
def get_foreign_players(connection, team_name):
	...
	result = None
	try:
		cursor.execute("SELECT player_name from Player WHERE team_name=\"" + team_name + "\" and nationality NOT LIKE 'India'")
		result = cursor.fetchall()
		return result
	except Error as e:
		print(f"The error '{e}' occurred")
...
def update_data(connection, dic, table_name,lis,id_for):
	...
	try:
		...
		for i in range(len(lis)-1):
			command +=  lis[i] + " = " + str(id_for[i]) + " and "
		command +=  lis[-1] + " = " + str(id_for[-1])
...
def get_graph_stats(connection):
	cursor = connection.cursor(dictionary=True)
	result = None
	try:
		cursor.execute("SELECT team_name,total_runs,total_wickets from player")
		result = cursor.fetchall()
		return result
	except Error as e:
		print(f"The error '{e}' occurred")
...
if __name__ == "__main__":
	...
	import pandas as pd
	teamData = get_graph_stats(connection)
	...
	for i in teamData:
		...
		z.append(float(list(i.values())[2]))

	data = pd.DataFrame({"X Value": y, "Y Value": z, "Category": x})
	groups = data.groupby("Category")
	for name, group in groups:
	    ax.plot(group["X Value"], group["Y Value"], marker="o", linestyle="", label=name)

	# ax.scatter(range(len(x)), range(len(y)), z)
	# ax.set(xticks=range(len(set(x))), xticklabels=set(x),
	#        yticks=range(len(y)), yticklabels=y) 
	plt.legend()
	fig.suptitle('Distribution of Players (Team Wise)', fontsize=20)
	plt.xlabel('Total Runs', fontsize=18)
	plt.ylabel('Total Wickets', fontsize=16)
	plt.savefig("./static/images/gr.png",dpi=200)
