...
with open(os.path.join(basedir, 'config.json'), 'r',encoding="utf8") as c:
...
@app.route("/writer")
def writer():
    return render_template('writer.html',params=params)
...
def edit(sno):
    if ('user' in session and session['user'] == params['admin_user']):
        if request.method =='POST':
            ...
            date=datetime.now()
            if sno=='0':
                post=Posts(title=title,tagline=tline,slug=slug,content=content,imgfile=imgfile,date=date)
                db.session.add(post)
                db.session.commit()
            else:
