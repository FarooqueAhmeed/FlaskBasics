...
def add():
    if request.method == 'POST':
        ...
        if data:
            ...
            db.cur.execute(
                "CREATE TABLE {0} (status varchar(10) DEFAULT 'active',srno int primary key auto_increment,name varchar(30),title varchar(30),company varchar(40),phone1 varchar(14),phone2 varchar(14),address varchar(90),grps varchar(25),email varchar(40),website varchar(40))".format(uname))
            db.con.commit()
            session['user'] = uname
            session['authuser'] = 1
            return redirect('/admin')
...
def addContact():
    if request.method == 'POST':
        ...
        db.cur.execute("INSERT INTO {0}(name,title,phone1,phone2,email,company,website,address,grps) VALUES('{1}',"
...
def updateContact(id):
    if request.method == 'POST':
        ...
        db.cur.execute("UPDATE {0} SET name='{1}',title='{2}',phone1='{3}',phone2='{4}',email='{5}',company='{6}',"
                       "website='{7}', "
                       "address='{8}',grps='{9}' WHERE srno='{10}'".format
